...
Bitcoin <-  ts(log(BITCOIN$Close), start = 2014, frequency = 1)

plot(Bitcoin, type="l", main="Logaritmos do Preço do Bitcoin", ylab="Log Preço", xlab="Data", col="Pink")
...
IntegradaOrdem1 <- ts(IntOrdem1, start = 2014, frequency = 1)
...
#Estima o AR3 e salva os resultados como AR3
#COMPLETAR ATÉ AR21

estimacoes <- list(AR1, AR2, AR3, AR4, AR5, AR6, AR7, AR8, AR9, AR10)      #Cria uma lista com os estimadores
...
Modelo <- c("AR1", "AR2","AR3","AR4","AR5","AR6","AR7","AR8","AR9","AR10")   #cria coluna com nome dos modelos
...
AR5 <- arima(IntegradaOrdem1, order = c(5,1,0))
MA5 <- arima(IntegradaOrdem1, order = c(0,1,5))
AR6 <- arima(IntegradaOrdem1, order = c(6,1,0))
MA6 <- arima(IntegradaOrdem1, order = c(0,1,6))
AR7 <- arima(IntegradaOrdem1, order = c(7,1,0))
MA7 <- arima(IntegradaOrdem1, order = c(0,1,7))
AR8 <- arima(IntegradaOrdem1, order = c(8,1,0))
MA8 <- arima(IntegradaOrdem1, order = c(0,1,8))
AR9 <- arima(IntegradaOrdem1, order = c(9,1,0))
MA9 <- arima(IntegradaOrdem1, order = c(0,1,9))
AR10 <- arima(IntegradaOrdem1, order = c(10,1,0))
MA10 <- arima(IntegradaOrdem1, order = c(0,1,10))
